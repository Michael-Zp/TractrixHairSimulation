...
void SmallEmulationSimulatedHair::InitializeSharedBuffers(ID3D11Device * device, ID3D11DeviceContext * context)
{
...
void SmallEmulationSimulatedHair::InitializeSplineRenderItem(ID3D11Device * device, ID3D11DeviceContext * context)
{
	...
	mSplineRenderItem.mVertexShader = new VertexShader(L"./Shader/vgpEmulationSplines.hlsl", "HairVS", true);
	mSplineRenderItem.mVertexShader->prepare(device);

	mSplineRenderItem.mGeometryShader = new GeometryShader(L"./Shader/vgpEmulationSplines.hlsl", "HairGS", true);
	mSplineRenderItem.mGeometryShader->prepare(device);

	mSplineRenderItem.mPixelShader = new PixelShader(L"./Shader/vgpEmulationSplines.hlsl", "HairPS", true);
	mSplineRenderItem.mPixelShader->prepare(device);
}

void SmallEmulationSimulatedHair::InitializeControlPolygonRenderItem(ID3D11Device * device, ID3D11DeviceContext * context)
{
	mControlPolygonRenderItem.mVertexShader = new VertexShader(L"./Shader/vgpEmulationControlPolygon.hlsl", "HairVS", true);
	mControlPolygonRenderItem.mVertexShader->prepare(device);

	mControlPolygonRenderItem.mGeometryShader = new GeometryShader(L"./Shader/vgpEmulationControlPolygon.hlsl", "HairGS", true);
	mControlPolygonRenderItem.mGeometryShader->prepare(device);

	mControlPolygonRenderItem.mPixelShader = new PixelShader(L"./Shader/vgpEmulationControlPolygon.hlsl", "HairPS", true);
	mControlPolygonRenderItem.mPixelShader->prepare(device);
}



SmallEmulationSimulatedHair::SmallEmulationSimulatedHair(ID3D11Device *device, ID3D11DeviceContext *context, SmallEmulationSimulation *simulation)
{
...
SmallEmulationSimulatedHair::~SmallEmulationSimulatedHair()
{
	free(mSimulation);
}

void SmallEmulationSimulatedHair::Draw(float deltaTime, ID3D11DeviceContext *context)
{
	if (!mIsUpdated)
	{
		DebugBreak();
...
void SmallEmulationSimulatedHair::DrawSplines(const float deltaTime, ID3D11DeviceContext * context)
{
...
void SmallEmulationSimulatedHair::DrawControlPolygon(const float deltaTime, ID3D11DeviceContext * context)
{
	context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
...
void SmallEmulationSimulatedHair::UpdateCamera(ID3D11DeviceContext *context, XMMATRIX view, XMMATRIX proj)
{
