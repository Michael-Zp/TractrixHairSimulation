...
SmallEmulationSimulation::SmallEmulationSimulation(ID3D11Device *device, ID3D11DeviceContext *context, PropertiesConstBuf props, XMFLOAT4 strandColor, Configuration config)
{
	std::unique_ptr<HairLoader> hairLoader = nullptr;

	if (config == SmallEmulationSimulation::Configuration::Mohawk || config == SmallEmulationSimulation::Configuration::Short)
	{
		std::string path = "";
		switch (config)
		{
		case SmallEmulationSimulation::Configuration::Mohawk:
			path = "./HairData/Ratboy/Ratboy_mohawk.tfx";
			break;
		case SmallEmulationSimulation::Configuration::Short:
	...
	switch (config)
	{
	case SmallEmulationSimulation::Configuration::Random1k16:
		mStrandsCount = 1000;
		mNumberOfSegments = 15;
		config = SmallEmulationSimulation::Configuration::Random;
		break;

	case SmallEmulationSimulation::Configuration::Random10k16:
		mStrandsCount = 10000;
		mNumberOfSegments = 15;
		config = SmallEmulationSimulation::Configuration::Random;
		break;

	case SmallEmulationSimulation::Configuration::Random1k32:
		mStrandsCount = 1000;
		mNumberOfSegments = 31;
		config = SmallEmulationSimulation::Configuration::Random;
		break;

	case SmallEmulationSimulation::Configuration::Random10k32:
		mStrandsCount = 10000;
		mNumberOfSegments = 31;
		config = SmallEmulationSimulation::Configuration::Random;
		break;
	...
	std::vector<SmallEmulationSimulation::Strand> strands;
	...
	//for (int i = 0; i < 200; i++)
	{
		...
		else
		{
			...
			switch (config)
			{
			case SmallEmulationSimulation::Configuration::Z4Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0)
				...
				break;
			case SmallEmulationSimulation::Configuration::Z12Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0),
				...
				basePoint = XMFLOAT3(0, 1.25, 0);
				break;
			case SmallEmulationSimulation::Configuration::Z31Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0),
				...
				basePoint = XMFLOAT3(0, 1.25, 0);
				break;
			case SmallEmulationSimulation::Configuration::Random:
			{
	...
	mComputeShader = new ComputeShader(L"./Shader/cSmallEmulationSimulation.hlsl", "Simulation", true);
	mComputeShader->prepare(device);
}


SmallEmulationSimulation::~SmallEmulationSimulation()
{

}

void SmallEmulationSimulation::Simulate(const float deltaTime, ID3D11DeviceContext *context)
{
