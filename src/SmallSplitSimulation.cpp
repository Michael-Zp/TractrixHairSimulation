...
SmallSplitSimulation::SmallSplitSimulation(ID3D11Device *device, ID3D11DeviceContext *context, PropertiesConstBuf props, XMFLOAT4 strandColor, Configuration config)
{
	std::unique_ptr<HairLoader> hairLoader = nullptr;

	if (config == SmallSplitSimulation::Configuration::Mohawk || config == SmallSplitSimulation::Configuration::Short)
	{
		std::string path = "";
		switch (config)
		{
			case SmallSplitSimulation::Configuration::Mohawk:
				path = "./HairData/Ratboy/Ratboy_mohawk.tfx";
				break;
			case SmallSplitSimulation::Configuration::Short:
	...
	switch (config)
	{
	case SmallSplitSimulation::Configuration::Random1k16:
		mStrandsCount = 1000;
		mNumberOfSegments = 15;
		config = SmallSplitSimulation::Configuration::Random;
		break;

	case SmallSplitSimulation::Configuration::Random10k16:
		mStrandsCount = 10000;
		mNumberOfSegments = 15;
		config = SmallSplitSimulation::Configuration::Random;
		break;

	case SmallSplitSimulation::Configuration::Random1k32:
		mStrandsCount = 1000;
		mNumberOfSegments = 31;
		config = SmallSplitSimulation::Configuration::Random;
		break;

	case SmallSplitSimulation::Configuration::Random10k32:
		mStrandsCount = 10000;
		mNumberOfSegments = 31;
		config = SmallSplitSimulation::Configuration::Random;
		break;
	...
	std::vector<SmallSplitSimulation::Strand> strands;
	std::vector<SmallSplitSimulation::PhysicalStrand> physicalStrands;
	...
	for (int i = 0; i < strandPoints.size(); i++)
	{
		...
		else
		{
			...
			switch (config)
			{
			case SmallSplitSimulation::Configuration::Z4Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0)
				...
				break;
			case SmallSplitSimulation::Configuration::Z12Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0),
				...
				basePoint = XMFLOAT3(0, 1.25, 0);
				break;
			case SmallSplitSimulation::Configuration::Z31Points:
				myDirections = {
					XMFLOAT3(0, -1, 0),
					XMFLOAT3(1, 0, 0),
					XMFLOAT3(0, -1, 0),
				...
				basePoint = XMFLOAT3(0, 1.25, 0);
				break;
			case SmallSplitSimulation::Configuration::Random:
			{
	...
	mComputeShader = new ComputeShader(L"./Shader/cSmallSplitSimulation.hlsl", "Simulation", true);
	mComputeShader->prepare(device);
}


SmallSplitSimulation::~SmallSplitSimulation()
{

}

void SmallSplitSimulation::Simulate(const float deltaTime, ID3D11DeviceContext *context)
{
